...
namespace Glingener\Sniffs\File;
...
/**
 * Checks the length of lines in the code.
 *
 * This just extends the Slevomat LineLengthSniff to ignore namespace statements.
 *
 * Original Sniff:
 * https://github.com/slevomat/coding-standard/blob/master/SlevomatCodingStandard/Sniffs/Files/LineLengthSniff.php
 *
 */
class LineLengthSniff implements Sniff
{
	...
	private function checkLineLength(File $phpcsFile, int $pointer): void
	{
		...
		if ($this->ignoreImports) {
			$namespace = NamespaceHelper::getAllNamespacesPointers($phpcsFile);

			foreach( $namespace as $namespacePointer ) {
				if ( $tokens[$namespacePointer]['line'] === $tokens[$pointer]['line'] ) {
					return;
				}
			}

			if ( in_array($pointer, $namespace, true) ) {
				return;
			}
